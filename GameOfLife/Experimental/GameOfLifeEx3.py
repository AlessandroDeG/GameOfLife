...
class Cell:
    ...
    ADJACENCY_DISTANCE=4  #maybe increment it for fun  ,   = cell field of view
    ...
    def checkStatus(self):
        ...
        step = Cell.size
        #for dx in range(lower, upper, step):
        lower-=Cell.size*ADJACENCY_DISTANCE
        upper+=Cell.size*ADJACENCY_DISTANCE
        for dy in range(lower, upper, step):
            ...
            while(aliveInGeneration==0):
                if(Cell.generationsQueue):
                    ...
                    for cell in generation:
                        if(cell.alive and cell.age>=genAge):
                            aliveInGeneration+=1
...
while(True):
    ...
    if(currentTime>=epochTime):
        ...
        print(" BORN-DIFF="+str(len(Cell.currentGenerationList)-lastGenBorn), end="")
        
        lastGenBorn=len(Cell.currentGenerationList)
        if(Cell.currentGenerationList):
            Cell.generationsQueue.append(Cell.currentGenerationList)
        ...
        print(" OLD-GEN-AGE="+str(len(Cell.generationsQueue)), end="")

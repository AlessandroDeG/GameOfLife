...
class Processor(multiprocessing.Process):
    def __init__(self, queue, idx , n , allCellsMap):
    ...
    def run(self):
        ...
        cellskeyList = list[self.allCellsMap.keys()]
        size = len(cellskeyList)
        rest = size%n
        batchSize=size//n
        
        for i in range(idx*batchSize,(idx+1)*batchSize):
            cell = allCellsMap[cellskeyList[i]]
    ...
    while(True):
        ...
        if(currentTime>=epochTime):
            ...
            for i in range(0, NUMBER_OF_PROCESSES):
                    p=Processor(queue=q, idx=i , n=NUMBER_OF_PROCESSES, Cell.allCellsMap)
                    p.start()
                    processes.append(p)            
            ...
            totalStatusTime = (time.time() - start)*1000
            
            """
            cellsList = list(Cell.allCellsMap.keys())
            l = len(cellsList)
            N_PROCESS=8
            lP=l//N_PROCESS
            rest = l%N_PROCESS
            ...
            cellsLists=[]
            last=0
            #for partitions in range(0,N_PROCESS):
            p=N_PROCESS
            for i in range(0,l):
                cellsInP=[]
                if(p!=lp*N_PROCESS)
                if(i==p-1):
                    cells.append(cellsInP)
                    cellsInP.clear()
                    p+=N_PROCESS
                cellsInP.append(cells[i]
            """   

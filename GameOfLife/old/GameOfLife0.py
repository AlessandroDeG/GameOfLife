...
class Cell:
    ...
    def fillAdjacent(Cell, x, y, alive): 
        adjacent={"adjacentList" : [],"aliveNumber" : 0,"alive" : alive, "self" : None}
            ...
            for dy in range(-Cell.ADJACENCY_DISTANCE*Cell.size, (Cell.ADJACENCY_DISTANCE+1)*Cell.size, Cell.size):
                ...
                if(dx != 0 or dy!=0):
                    ...
                    if(alive): #center cell is alive
                        ...
                        if(not Cell.exist(x+dx,y+dy)["exist"]):  #adj cell not exist (and is alive or not alive)
                            Cell.spawn(x+dx,y+dy, False)  
                            (adjacent["adjacentList"]).append(Cell.exist(x+dx,y+dy)["cell"]) 
                            
                        elif(Cell.exist(x+dx,y+dy)["exist"]):    
                            if(Cell.exist(x+dx,y+dy)["alive"]):  #adj exist and is alive
                                (adjacent["adjacentList"]).append(Cell.exist(x+dx,y+dy)["cell"])
                                adjacent["aliveNumber"] = adjacent["aliveNumber"]+1

                    if(not alive ):  #center cell is not alive
                      
                        if(Cell.exist(x+dx,y+dy)["exist"]):    
                            if(Cell.exist(x+dx,y+dy)["alive"]):  #adj exist and is alive
                                (adjacent["adjacentList"]).append(Cell.exist(x+dx,y+dy)["cell"])
                                adjacent["aliveNumber"] = adjacent["aliveNumber"]+1
...
while(True):
    ...
    #player spawning
        ...
        for cell in Cell.allCellsList:
            ...
            ##dead->alive
            elif((not status["alive"]) and status["aliveNumber"]==3):
                #print(str(status["aliveNumber"])+"==3 : dead ->alive")
                flippingCells.append(status["self"])
           
        
            #dead->useless        
            elif((not status["alive"]) and status["aliveNumber"]==0):
                #print(str(status["aliveNumber"])+"==0 : dead ->useless")
                uselessCells.append(status["self"])
                
        print("CHECKSTATUS TIME: "+str(totalStatusTime)+" ms \r", end="")
